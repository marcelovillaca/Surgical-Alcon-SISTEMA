...
const target = path.join(__dirname, '..', 'src', 'hooks', 'useDashboardData.ts');
...
const content = [
    ...
    "          .select('fecha,cliente,cod_cliente,ciudad,linea_de_produto,codigo_produto,produto,costo,total,monto_usd,vendedor,mercado')",
    ...
    "    effectiveCosts.forEach(r => { const key = normStr(r.item_name || ''); if (key) costRefMap[key] = Number(r.costo_unitario || 0); });",
    ...
    "      if (!allLines && !normalizedFilterLines.includes(normalizeLine(s.linea_de_produto))) return false;",
    "      if (filterVendedor !== 'todos' && s.vendedor.trim().toLowerCase() !== filterVendedor) return false;",
    "      if (!includePublic) {",
    "        const mStr = (s.mercado || '').toString().toLowerCase();",
    "        if (mStr.includes('p\\u00fablico') || mStr.includes('publico')) return false;",
    ...
    "      if (!allLines && !normalizedFilterLines.some(fl => normalizeLine(t.linea_de_produto) === fl)) return false;",
    ...
    "    const totalCosto = sales.reduce((s, r) => {",
    "      const unitCost = costRefMap[normStr(r.codigo_produto)] ?? costRefMap[normStr(r.produto)];",
    "      return s + ((unitCost != null && unitCost > 0) ? unitCost * Number(r.total) : Number(r.costo));",
    "    }, 0);",
    ...
    "    const uniqueProducts = new Set(sales.map(s => s.produto)).size;",
    ...
    "      cur.ventas += Number(s.monto_usd); cur.costo += Number(s.costo);",
    ...
    "",
    "    const uniqueNL = Array.from(new Set([...Array.from(lineNames.keys()), ...targetsRaw.filter(t => t.anio === year).map(t => normalizeLine(t.linea_de_produto))]));",
    ...
    "      const lineTargets = targets.filter(t => normalizeLine(t.linea_de_produto) === norm);",
    "      let lineMeta = 0;",
    ...
    "    sales.forEach(s => { prodMap.set(s.produto.trim(), (prodMap.get(s.produto.trim()) || 0) + Number(s.monto_usd)); });",
    ...
    "      monthlySales, productMix, marginTrend,",
